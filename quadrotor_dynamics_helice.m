function dXdt = quadrotor_dynamics_helice(t, X, fis, m, Ix, Iy, Iz, g, b, d, l, Jr,...
...
% Desired position (helix)
xd = R*cos(omega*t);
yd = R*sin(omega*t);
zd = z0 + h_speed*t;

% Desired velocity (derivative)
dxd = -R*omega*sin(omega*t);
dyd = R*omega*cos(omega*t);
dzd = h_speed;

% Desired acceleration (second derivative)
ddxd = -R*omega^2*cos(omega*t);
ddyd = -R*omega^2*sin(omega*t);
ddzd = 0;

% Desired yaw points along velocity vector (tangent to helix)
psi_d = atan2(dyd, dxd);  % Point in direction of motion
% psi_d = 0;  % Uncomment for constant yaw
