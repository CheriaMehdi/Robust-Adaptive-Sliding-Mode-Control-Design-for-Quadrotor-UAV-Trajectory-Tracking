function dXdt = quadrotor_dynamics(t, X, fis, m, Ix, Iy, Iz, g, b, d, l, Jr, ...
    lambda, k1_fixed, k2, kp_xy, kd_xy, kp_z, kd_z, controller_type, traj)
% quadrotor_dynamics - dynamics + inner SMC/AFGS controller
% traj: struct with fields a, b_val, wz (desired trajectory parameters)
...
% Desired Trajectory (use parameters passed from main)
a     = traj.a; b_val = traj.b; wz = traj.wz;
xd    = a*sin(wz*t);           yd = b_val*sin(2*wz*t); zd = traj.zd;
...
% Desired roll and pitch (safe asin arguments)
...
% Attitude errors (desired rates assumed zero)
...
if strcmp(controller_type,'AFGS')
    % NOTE: evalfis expects a row vector [in1 in2]
    k1_phi   = max(evalfis(fis, [s_phi,   de_phi]),  0.01);
    k1_theta = max(evalfis(fis, [s_theta, de_theta]), 0.01);
    k1_psi   = max(evalfis(fis, [s_psi,   de_psi]),   0.01);
else
    % Classical SMC: fixed gain (passed separately from main)
    k1_phi = k1_fixed; k1_theta = k1_fixed; k1_psi = k1_fixed;
end

% Dynamics coefficients (paper eq.4 notation)
a1 = (Iy - Iz)/Ix;   a2 = Jr/Ix;
a3 = (Iz - Ix)/Iy;   a4 = Jr/Iy;
a5 = (Ix - Iy)/Iz;

% Temporary torques (Omega_d = 0 in temp)
...
Omega_sq = max(Omega_sq, 0);  % avoid negative before sqrt

% Compute Omega_d (disturbance/prop rotation effect) as in your original formulation
Omega_d = -sqrt(Omega_sq(1)) + sqrt(Omega_sq(2)) - sqrt(Omega_sq(3)) + sqrt(Omega_sq(4));

% Final torques with gyro coupling
...
% Rotational accelerations (paper equations)
...
% State derivatives
% order: pos_dot (vel), att_dot (angvel), vel_dot (ddx ddy ddz), angvel_dot (ddphi ddtheta ddpsi)
dXdt = [ vel;
         angvel;
         ddx; ddy; ddz;
         ddphi; ddtheta; ddpsi ];
